...
<Meta title="config/Snapshot Path" />

# Customizing Snapshot Path

[vitest-plugin-vis] allows you to customize the snapshot path through the `snapshotRootDir`, `snapshotSubpath`, and `snapshotKey` options.
...
When you run the test locally, the image snapshot will be saved in the following path:
...
When running locally, the `snapshotRootDir` is default to `__vis__/local`.
...
> `MyComponent/className/can-customize-className`: snapshot ID
