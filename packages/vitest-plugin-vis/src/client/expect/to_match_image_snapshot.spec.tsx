...
it('accepts `baseElement` (same as body)', async ({ expect }) => {
	...
	const { baseElement } = await render(<div data-testid="subject">hello</div>)
...
it('fails when the image is different', async ({ expect }) => {
	...
	await render(<div data-testid="subject">{hasImageSnapshot ? 'Hello' : 'World'}</div>)
	const subject = page.getByTestId('subject')
	if (!hasImageSnapshot) {
		await expect(subject).toMatchImageSnapshot()
		return
...
describe(`${setAutoSnapshotOptions.name}()`, () => {
	...
	it('can enable auto snapshot from nested beforeEach', async () => {
