...
- Configure visual testing in `.storybook/preview.ts` or `vitest.setup.ts`, depending on whether you are writing your stories in CSF 3 or CSF Next.
...
you don't need to setup the `vitest.setup.ts` file.
Instead, you need to setup the visual testing in `.storybook/preview.ts`.
...
In `.storybook/preview.ts` or `vitest.setup.ts`, you can customize how the automatic snapshots are taken.

The `addonVis()` used in `.storybook/preview.ts` and `vis.setup()` used in `vitest.setup.ts` have the same options.
Here, we will use the `addonVis()` options as an example.
