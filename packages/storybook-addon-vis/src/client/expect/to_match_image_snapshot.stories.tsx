		...
		await expect(subject)
			...
			.then(
				...
				(error) => {
					...
					expect(error.message).toMatch(/The image size changed from 128x128 to \d{2}x\d{2}/)
				},
			)
	},
}
	...
	async play({ canvas, loaded: { hasImageSnapshot } }) {
		...
		await expect(subject)
			...
			.then(
				...
				(error) => {
					expect(error.message).toMatch(/^Snapshot .* mismatched/)
					expect(error.message).toMatch(/The image size changed from 64x64 to \d{3}x\d{3}/)
				},
			)
	},
}
